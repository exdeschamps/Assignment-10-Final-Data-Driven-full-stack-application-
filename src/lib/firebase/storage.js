...
import { updateAlbumImageReference } from "@/src/lib/firebase/firestore"; // helper to update Firestore doc with image URL

export async function updateAlbumImage(albumId, image) {
  try {
    if (!albumId) {
      throw new Error("No album ID has been provided."); // validate input
    }
    ...
    const publicImageUrl = await uploadImage(albumId, image); // upload and get public URL
    await updateAlbumImageReference(albumId, publicImageUrl); // update Firestore album doc
...
async function uploadImage(albumId, image) {
  const filePath = `images/${albumId}/${image.name}`; // storage path for the file
